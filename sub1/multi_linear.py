...
for i,dataList in enumerate(csv_data):
   if i!=0:
      X.append([float(dataList[1]),float(dataList[2]),float(dataList[3])])
      Y.append(float(dataList[4]))
...
# 파일명: model.clf
# """
pickle.dump(lrmodel,open('model.clf','wb'))
...
def prediction(X,beta_x):
    # 예측 값을 계산하는 식을 만든다.
    equation = np.dot(X,beta_x)
...
# Req. 3-2-1. 모델 학습
N_beta_x, N_beta_3  = N_LinearRegression(X,Y,iteration,learning_rate)

# Req. 3-2-2. 학습된 가중치 저장
print("\nN_LinearRegression의 결과물")
print("N_beta_0: %f" % N_beta_x[0])
print("N_beta_1: %f" % N_beta_x[1])
print("N_beta_2: %f" % N_beta_x[2])
# print(N_beta_x)
print("N_beta_3: %f" % N_beta_3)
